...
import { useContentStats } from '../hooks/useAnalytics';
import {
  BarChart,
  Bar,
  XAxis,
...
import { formatRuntime, formatNumber } from '../hooks/useAnalytics';
...
const ContentStats: React.FC = () => {
  ...
  // Chart colors
  const COLORS = ['#aa5cc8', '#00a4dc', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#06b6d4', '#84cc16', '#f97316', '#ec4899'];

  // Content type distribution for pie chart
  const typeDistribution = contentStats.mostWatchedContent.reduce((acc, content) => {
            ...
            <BarChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              ...
              <Bar 
                ...
                name={
                  ...
                  sortBy === 'totalRuntime' ? 'Total Hours' : 'Unique Users'
                }
              />
            </BarChart>
          </ResponsiveContainer>
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-center space-x-2 text-sm text-gray-400">
                  <span className={`px-2 py-0.5 rounded text-xs ${
                    content.type === 'Movie' ? 'bg-blue-900/50 text-blue-300' :
                    content.type === 'Episode' ? 'bg-green-900/50 text-green-300' :
                    content.type === 'Audio' ? 'bg-purple-900/50 text-purple-300' :
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-jellyfin-blue">
              {formatNumber(contentStats.mostWatchedContent.reduce((sum, content) => sum + content.playCount, 0))}
            </div>
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-jellyfin-purple">
              {formatNumber(
                Math.round(contentStats.mostWatchedContent.reduce((sum, content) => sum + content.totalRuntime, 0) / 3600)
              )}h
            </div>
