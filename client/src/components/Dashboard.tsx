...
import { useDashboardOverview, useActivityTimeline, useActiveSessions } from '../hooks/useAnalytics';
        ...
        <section>
          <div className="chart-container">
            ...
            {sessionsLoading ? (
              ...
              <div className="error-state">
                <svg className="w-12 h-12 mx-auto mb-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2 2v-5m16 0h-6m-10 0h6m0 0v-5" />
                </svg>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3 max-h-32 overflow-y-auto">
                  {sessions.filter(s => s.NowPlayingItem).map((session) => (
                    <div key={session.Id} className="flex items-center justify-between p-3 bg-gray-700/50 rounded-lg">
                      ...
                      <div className="flex items-center space-x-2">
                        <span className={`status-badge ${
                          session.PlayState?.IsPaused ? 'bg-yellow-900/50 text-yellow-300' : 'status-active'
                        }`}>
                ...
                {overview?.topUsers.slice(0, 5).map((user, index) => (
                  <div key={user.id} className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-gray-400 text-sm">{formatRuntime(user.totalRuntime)}</div>
                    </div>
                  </div>
                ))}
              </div>
